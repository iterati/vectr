...
1 - Light sends [Handshake, Version, Check Val, Check Val]
2 - If version match and check vals match,
    UI sends [Handshake, Version, Check Val, Check Val]
    Light enters View Mode and disables light's button
3 - For all 128 mode bytes,
    UI sends [Write, Byte, Value, 0]
    Light and UI are now in sync
4 - All changes through the UI should send [Write, Byte, Value, 0] to keep in sync
5 - UI sends [Disconnect, 0, 0, 0] when finishes to re-enable button
